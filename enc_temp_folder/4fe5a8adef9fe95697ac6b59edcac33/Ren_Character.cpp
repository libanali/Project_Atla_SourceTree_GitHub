...
void ARen_Character::CanUseElementalAbility()
{
	...
	if (ManaStruct.CurrentMana > 0 && bCanCast && IsCombatModeOn)

	{
		ManaReduction = ManaStruct.CurrentMana / 1.5f;
		ManaStruct.CurrentMana = FMath::Clamp(ManaStruct.CurrentMana - ManaReduction, 0, ManaStruct.MaxMana);
		bCanCast = true;
